...
const ActivityGroupAddEdit = () => {
  ...
  useEffect(() => {
    if (activitygroupid) {
      axios
        .get(API.VIEW_ACTIVITYGROUP_API(activitygroupid))
  ...
  const handlSubmit = (e) => {
    ...
    if (!groupactivity) {
      ...
      if (!activitygroupid) {
        axios
          .post(API.ADD_ACTIVITYGROUP_API, {
            groupactivity,
            theme,
            phase,
          })
          .then(() => {
            setState({ groupactivity: " ", theme: " ", phase: " " });
          })
          .catch((err) => toast.error(err.response.data));
        toast.success("activity group successfully");
      } else {
        axios
          .put(API.UPDATE_ACTIVITYGROUP_API(activitygroupid), {
            groupactivity,
            theme,
            phase,
          })
