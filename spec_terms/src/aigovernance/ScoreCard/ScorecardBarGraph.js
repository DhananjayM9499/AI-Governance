...
const ScorecardBarGraph = ({ graphData }) => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      ...
      const evidenceremarkCount = labels.map(
        (thrustArea) =>
          countsGroupedByThrustArea[thrustArea].evidenceremarkCount
      );
      const assessmentremarkCount = labels.map(
        (thrustArea) =>
          countsGroupedByThrustArea[thrustArea].assessmentremarkCount
      );
      const auditremarkCount = labels.map(
        (thrustArea) => countsGroupedByThrustArea[thrustArea].auditremarkCount
      );
