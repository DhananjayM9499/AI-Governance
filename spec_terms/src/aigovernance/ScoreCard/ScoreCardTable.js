...
const ScoreCardTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setRespGroup(await fetchDataFromAPI(API.GET_RESPONSIBILITY_GROUP));
      setRespCenter(await fetchDataFromAPI(API.GET_RESPONSIBILITY_CENTER));
      setObjectType(await fetchDataFromAPI(API.GET_OBJECTTYPE_API));
      setObjectName(await fetchDataFromAPI(API.GET_DISTINCTOBJECTNAME_API));
      setProject(await fetchDataFromAPI(API.GET_DISTINCTPROJECT_API));
      setGroupName(await fetchDataFromAPI(API.GET_GOVERNANCE_API));
      setOrganizationComp(await fetchDataFromAPI(API.GET_DISTINCTCOMPANY_API));
    };
            ...
            <div>
              ...
              >
                ...
                {objectName.map((objname) => (
                  <option key={objname.resultid} value={objname.objectcode}>
                    {objname.objectcode}
                  </option>
                ))}
              </select>
